...
public class ClientCommand extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] astring) throws WrongUsageException {
        if(sender instanceof EntityPlayerSP) {
            ...
            else if(astring[0].equals("tunnel") && astring.length == 4){
            	if(Integer.parseInt(astring[1]) == 0){
            		VeinMiner.instance.tunnelOn = false;
            		senderPlayer.addChatMessage(new TextComponentTranslation("command.veinminerc.set.tunnelOff"));
            	}
            	else{
            		VeinMiner.instance.tunnelOn = true;
            		VeinMiner.instance.minerServer.getConfigurationSettings().setTunnelHeight(Integer.parseInt(astring[1]));
            		VeinMiner.instance.minerServer.getConfigurationSettings().setTunnelWidth(Integer.parseInt(astring[2]));
            		VeinMiner.instance.minerServer.getConfigurationSettings().setTunnelLength(Integer.parseInt(astring[3]));
            		senderPlayer.addChatMessage(new TextComponentTranslation("command.veinminerc.set.tunnelOn"));
            	}
            }
            else {
                throw new WrongUsageException("/veinminerc disabled/pressed/released/sneak/no_sneak");
